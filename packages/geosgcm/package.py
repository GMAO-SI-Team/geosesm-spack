...
class Geosgcm(CMakePackage):
    ...
    depends_on("fms precision=32,64 +quad_precision ~gfs_phys +openmp +pic constants=GEOS build_type=Release", when="@12: ~debug")
    depends_on("fms precision=32,64 +quad_precision ~gfs_phys +openmp +pic constants=GEOS build_type=Debug", when="@12: +debug")
